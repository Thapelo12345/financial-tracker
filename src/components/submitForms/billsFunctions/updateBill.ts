...
export async function UpdateBill(
  ...
  if (data) {
    ...
    const crrBill = currentUser.recurringBills.find((item:DataBaseBill) => item.id === bill.id);

    if(crrBill !== undefined){
    try{
     const getDocuments = await getDocs(collection(db, "users"));
     const matchingUser= getDocuments.docs.find((doc)=> doc.data().email === currentUser.email)
     ...
     if(matchingUser){
            ...
            .then(()=> {
                ...
                .then(()=> {
                const pos = currentUser.recurringBills.findIndex((item:DataBaseBill) => item.id === bill.id)
